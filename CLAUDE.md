...
- Keep the TypeScript and Python packages in feature parity; a change to `src/index.ts` usually needs the same change in `python/src/shortcutkit/__init__.py`, and tests in `test/shortcut.test.ts` assert exact counts (392 actions) that change only with a data refresh.
